...
import { Upload, Copy, Check, History, ExternalLink, Trash2, Loader2, ArrowRight, CheckCircle, XCircle, AlertTriangle, LogOut, User, Moon, Sun } from 'lucide-react';
...
const DARK = {
  ...
  scrollbar: '#2a2a2a',
  btnPrimary: '#6366f1', btnPrimaryText: '#ffffff', btnPrimaryHover: '#4f46e5',
  btnGhostText: '#94a3b8', btnGhostBorder: '#2a2a2a',
  cr: '99,102,241', emptyText: '#2a2a2a',
  toggleBg: '#1a1a1a', toggleText: '#64748b',
};
...
const LIGHT = {
  ...
  cr: '99,102,241', emptyText: '#e2e8f0',
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const p of pts) {
        ...
        if (dd < 80 && dd > 0) { const f = (80 - dd) / 80; p.x += dx / dd * f; p.y += dy / dd * f; }
        p.a += (p.base - p.a) * 0.05;
      }
      for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) {
        const dd = dist(pts[i].x, pts[i].y, pts[j].x, pts[j].y);
        if (dd < 120) {
          const op = (1 - dd / 120) * 0.12;
          const near = dist(pts[i].x, pts[i].y, mouse.x, mouse.y) < 150 || dist(pts[j].x, pts[j].y, mouse.x, mouse.y) < 150;
          ctx.beginPath(); ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y);
          ctx.strokeStyle = `rgba(${cr},${near ? op * 5 : op})`; ctx.lineWidth = near ? 0.8 : 0.4; ctx.stroke();
        }
      }
      for (const p of pts) {
        const near = dist(p.x, p.y, mouse.x, mouse.y) < 150;
        ctx.beginPath(); ctx.arc(p.x, p.y, near ? p.r * 2 : p.r, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${cr},${near ? Math.min(p.a * 4, 0.8) : p.a})`; ctx.fill();
      }
  ...
  return (
    <div style={{ position: 'fixed', bottom: '1.5rem', right: '1.5rem', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '0.6rem', maxWidth: '340px', width: '100%' }}>
      <AnimatePresence mode="popLayout">
        {toasts.map(toast => {
          const cfg = {
            success: { bg: t.successBg, border: t.successBorder, text: t.success, icon: <CheckCircle size={15} /> },
            error:   { bg: t.errorBg,   border: t.errorBorder,   text: t.error,   icon: <XCircle size={15} /> },
            warning: { bg: t.warningBg, border: t.warningBorder, text: t.warning, icon: <AlertTriangle size={15} /> },
          }[toast.type];
          return (
...
function ThemeToggle({ isDark, onToggle }: { isDark: boolean; onToggle: () => void }) {
  ...
  return (
    <button onClick={onToggle}
      style={{ display: 'flex', alignItems: 'center', gap: 6, background: t.toggleBg, border: `1px solid ${t.border}`, padding: '0.4rem 0.85rem', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '0.72rem', color: t.toggleText, borderRadius: '6px', transition: 'all 0.2s' }}>
    ...
    <div style={{ background: t.bg, minHeight: '100vh', color: t.text, fontFamily: "'Inter', sans-serif", overflowX: 'hidden', transition: 'background 0.3s' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.2rem 3rem', borderBottom: `1px solid ${t.border}`, position: 'sticky', top: 0, background: t.navBg, backdropFilter: 'blur(20px)', zIndex: 100, transition: 'background 0.3s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <ChainLogo size={36} color={t.accent} />
          <span style={{ fontSize: '1.05rem', fontWeight: 700, color: t.text2, letterSpacing: '-0.03em' }}>SiteSnap</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <ThemeToggle isDark={isDark} onToggle={onToggleTheme} />
          <button onClick={() => { sessionStorage.setItem('inApp','1'); onEnterApp(); }}
            style={{ background: t.accent, color: '#fff', border: 'none', padding: '0.5rem 1.2rem', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer', borderRadius: '6px', transition: 'background 0.2s' }}
      ...
      <section style={{ minHeight: '88vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '5rem 2rem 4rem', position: 'relative', zIndex: 1 }}>
        <HeroReveal delay={0} dir="down">
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: isDark ? 'rgba(99,102,241,0.1)' : 'rgba(99,102,241,0.08)', border: `1px solid rgba(99,102,241,0.3)`, padding: '0.4rem 1rem', borderRadius: '100px', fontFamily: "'Inter',sans-serif", fontSize: '0.72rem', color: t.accentText, marginBottom: '2rem', letterSpacing: '0.04em', fontWeight: 500 }}>
        ...
        <HeroReveal delay={100} dir="up">
          <h1 style={{ fontSize: 'clamp(2.8rem,7vw,5.5rem)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.05em', marginBottom: '1.5rem', maxWidth: 800, color: t.text2 }}>
            ...
            <span style={{ color: t.accent }}>live in seconds.</span>
          </h1>
        </HeroReveal>
        <HeroReveal delay={200} dir="up">
          <p style={{ fontSize: '1.1rem', color: t.textMuted, maxWidth: 480, lineHeight: 1.8, marginBottom: '2.5rem', fontWeight: 400 }}>Upload your WordPress ZIP and get a permanent shareable link — without touching a server.</p>
        </HeroReveal>
        <HeroReveal delay={320} dir="up">
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
            <button onClick={() => { sessionStorage.setItem('inApp','1'); onEnterApp(); }}
              style={{ background: t.accent, color: '#fff', border: 'none', padding: '0.85rem 2rem', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', borderRadius: '8px', display: 'inline-flex', alignItems: 'center', gap: 8, transition: 'all 0.2s', boxShadow: `0 4px 24px rgba(99,102,241,0.3)` }}
              onMouseOver={e => { e.currentTarget.style.background = t.accentHover; e.currentTarget.style.transform = 'translateY(-1px)'; }}
            ...
            <button onClick={() => document.getElementById('hp-how')?.scrollIntoView({ behavior: 'smooth' })}
              style={{ background: 'transparent', color: t.textMuted, border: `1px solid ${t.border2}`, padding: '0.85rem 2rem', fontFamily: "'Inter',sans-serif", fontWeight: 500, fontSize: '0.95rem', cursor: 'pointer', borderRadius: '8px', transition: 'all 0.2s' }}
          ...
          <div style={{ display: 'flex', gap: '3rem', marginTop: '4rem', paddingTop: '3rem', borderTop: `1px solid ${t.border}` }}>
            {[['ZIP', 'One file to deploy'], ['∞', 'Permanent links'], ['0', 'Setup required']].map(([n, l]) => (
              <div key={l} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: 800, color: t.accent, letterSpacing: '-0.04em' }}>{n}</div>
      ...
      <section style={{ padding: '6rem 3rem', position: 'relative', zIndex: 1, borderTop: `1px solid ${t.border}` }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: '1px', background: t.border, border: `1px solid ${t.border}`, borderRadius: '12px', overflow: 'hidden' }}>
          {[{ icon: '⚡', title: 'Instant Deploy', desc: 'Drop your ZIP and go. No config, no CLI.' }, { icon: '🔗', title: 'Permanent Links', desc: 'Every deploy gets a URL that never expires.' }, { icon: '📁', title: 'Deploy History', desc: 'All your deployments in one clean list.' }, { icon: '🔒', title: 'Secure Auth', desc: 'Encrypted passwords. JWT protected routes.' }].map((f, i) => (
            <Reveal key={f.title} dir="up" delay={i * 60}>
              <div style={{ background: t.cardBg, padding: '2rem 1.8rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', transition: 'background 0.2s', height: '100%' }}
          ...
          {[{ n: '01', title: 'Export from WordPress', desc: 'Use Simply Static plugin to export your site as a ZIP file.' }, { n: '02', title: 'Upload to SiteSnap', desc: 'Drop your ZIP. Upload goes directly to secure cloud storage.' }, { n: '03', title: 'Share your link', desc: 'Get a permanent URL instantly. Share it with anyone.' }].map((s, idx, arr) => (
            <Reveal key={s.n} dir="left" delay={idx * 100}>
              <div style={{ display: 'flex', gap: '1.5rem', padding: '2rem 0', borderBottom: idx < arr.length - 1 ? `1px solid ${t.border}` : 'none' }}>
                <div style={{ fontSize: '0.7rem', color: t.accent, minWidth: 32, paddingTop: 3, fontWeight: 700, fontFamily: "'Inter',monospace" }}>{s.n}</div>
                <div>
                  <div style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.4rem', color: t.text2 }}>{s.title}</div>
                  <div style={{ fontSize: '0.88rem', color: t.textMuted, lineHeight: 1.6 }}>{s.desc}</div>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      <section style={{ padding: '7rem 3rem', textAlign: 'center', position: 'relative', zIndex: 1, borderTop: `1px solid ${t.border}` }}>
        <Reveal dir="up">
          <div style={{ maxWidth: 560, margin: '0 auto', background: isDark ? 'rgba(99,102,241,0.05)' : 'rgba(99,102,241,0.04)', border: `1px solid rgba(99,102,241,0.2)`, borderRadius: '16px', padding: '4rem 3rem' }}>
            ...
            <button onClick={() => { sessionStorage.setItem('inApp','1'); onEnterApp(); }}
              style={{ background: t.accent, color: '#fff', border: 'none', padding: '0.9rem 2.2rem', fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', borderRadius: '8px', display: 'inline-flex', alignItems: 'center', gap: 8, transition: 'all 0.2s', boxShadow: `0 4px 24px rgba(99,102,241,0.3)` }}
              onMouseOver={e => { e.currentTarget.style.background = t.accentHover; e.currentTarget.style.transform = 'translateY(-1px)'; }}
              onMouseOut={e => { e.currentTarget.style.background = t.accent; e.currentTarget.style.transform = 'none'; }}>
      ...
      <footer style={{ borderTop: `1px solid ${t.border}`, padding: '1.5rem 3rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <ChainLogo size={24} color={t.accent} />
          <span style={{ fontSize: '0.85rem', fontWeight: 600, color: t.text2 }}>SiteSnap</span>
        </div>
...
// ── Auth Page ─────────────────────────────────────────────────────────────────
function AuthPage({ isDark, onToggleTheme, onAuth }: { isDark: boolean; onToggleTheme: () => void; onAuth: (user: AuthUser, token: string) => void }) {
  const t = isDark ? DARK : LIGHT;
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div style={{ background: t.panelBg, border: `1px solid ${t.border}`, borderRadius: '16px', padding: '2.5rem 2.2rem', boxShadow: isDark ? '0 24px 64px rgba(0,0,0,0.5)' : '0 24px 64px rgba(0,0,0,0.08)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              ...
              <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: t.text2, letterSpacing: '-0.04em', margin: '1rem 0 0.4rem' }}>
            ...
            <button onClick={handleSubmit} disabled={loading}
              ...
              onMouseOver={e => { if (!loading) e.currentTarget.style.background = t.accentHover; }}
              onMouseOut={e => { e.currentTarget.style.background = t.accent; }}>
          ...
          <p style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.68rem', color: t.textDim }}>🔒 Passwords encrypted · Data never sold</p>
        </motion.div>
      </div>
    </div>
  );
...
export default function App() {
  const [isDark, setIsDark] = useState<boolean>(() => {
    const s = localStorage.getItem('sitesnap-theme'); return s === null ? true : s === 'dark';
  });
  ...
  const onDrop = useCallback(async (accepted: File[], rejected: any[]) => {
    ...
    try {
      ...
      setIsUploading(false); setUploadProgress(0);
      addToast('error', 'Upload failed', e.message || 'Something went wrong. Try again.');
    }
  }, [addToast, authFetch]);
  ...
  if (!authUser) return <AuthPage isDark={isDark} onToggleTheme={toggleTheme} onAuth={handleAuth} />;
    ...
    <div style={{ minHeight: '100vh', background: t.bg, color: t.text, fontFamily: "'Inter', sans-serif", transition: 'background 0.3s' }}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .dep-list::-webkit-scrollbar-thumb{background:${t.scrollbar};border-radius:4px;}
      `}</style>
      <ConstellationCanvas isDark={isDark} />
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2.5rem', borderBottom: `1px solid ${t.border}`, background: t.navBg, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 50, transition: 'background 0.3s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
          <button onClick={() => { sessionStorage.removeItem('inApp'); setShowHome(true); }}
            style={{ background: 'transparent', color: t.textMuted, border: `1px solid ${t.border}`, padding: '0.38rem 0.85rem', fontFamily: "'Inter',sans-serif", fontSize: '0.78rem', fontWeight: 500, cursor: 'pointer', borderRadius: '6px', transition: 'all 0.2s' }}>← Back</button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <ChainLogo size={28} color={t.accent} />
            <span style={{ fontSize: '0.9rem', fontWeight: 700, color: t.text2, letterSpacing: '-0.03em' }}>SiteSnap</span>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 2rem', position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 800, letterSpacing: '-0.05em', color: t.text2, lineHeight: 1.05 }}>
            Drop your ZIP. <span style={{ color: t.accent }}>Own your link.</span>
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          ...
          <div style={{ background: t.panelBg, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '1.8rem' }}>
            ...
            <div {...getRootProps()} style={{
              height: 280, border: `2px dashed ${isDragActive ? t.accent : t.uploadBorder}`,
              background: isDragActive ? (isDark ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.04)') : t.uploadBg,
              ...
              <input {...getInputProps()} />
              {isUploading && (
                <div style={{ position: 'absolute', bottom: 0, left: 0, height: '3px', background: t.border, width: '100%' }}>
                  <motion.div initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} style={{ height: '100%', background: `linear-gradient(90deg, ${t.accent}, #818cf8)`, borderRadius: '0 2px 2px 0' }} />
                </div>
              )}
                    ...
                    <motion.div key={site.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.65rem' }}>
                        ...
                        <a href={site.url} target="_blank" rel="noreferrer"
                          style={{ width: 28, height: 28, background: t.accent, borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', transition: 'background 0.2s' }}
                          onMouseOver={e => (e.currentTarget.style.background = t.accentHover)} onMouseOut={e => (e.currentTarget.style.background = t.accent)}>
                          <ExternalLink size={12} color="#fff" />
                        </a>
                      </div>
                      <div style={{ background: t.depUrl, border: `1px solid ${t.depUrlBorder}`, padding: '0.4rem 0.7rem', fontFamily: 'monospace', fontSize: '0.62rem', color: t.textDim, marginBottom: '0.65rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', borderRadius: '6px' }}>
                        {site.url}
                      </div>
